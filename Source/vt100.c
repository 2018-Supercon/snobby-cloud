...
void video_gotoxy(int8_t x, int8_t y){
	...
	if (cy >= DISP_BUFFER_HIGH){
		cy = DISP_BUFFER_HIGH-1;
	}
	CURSOR_INVERT();
	}
...
void video_putcxy(int8_t x, int8_t y, int8_t c){
	if (x < 0 || x >= DISP_BUFFER_WIDE){
		return;
	}
	if (y < 0 || y >= DISP_BUFFER_HIGH){
		return;
	}
...
void video_putline(int8_t y, int8_t *str){
	...
	if (revvideo){
		video_invert_range(0, y, DISP_BUFFER_WIDE);
	}
}
